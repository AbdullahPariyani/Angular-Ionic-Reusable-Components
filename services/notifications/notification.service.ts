...
export class NotificationService {
	...
	constructor(private _httpService: HttpService, private _local: LocalStorageService, private router: Router) {}
	...
	async OneSignalInit(): Promise<void> {
		...
		await OneSignal.initialize(APP_CONFIG.ONE_SIGNAL_NOTIFICATION_ID);
		...
		await OneSignal.Notifications.requestPermission().then((accepted: boolean) => {
	...
	private async getOneSignalSubscriptionID() {
		this.onsSignalSubscriptionID = await OneSignal.User.pushSubscription.getIdAsync();
	}
	...
	private async askForPermission() {
		await OneSignal.Notifications.requestPermission(true).then((accepted: boolean) => {
			this.isSubscribedToPushNotifications = accepted;
			console.log('User accepted notifications: ' + accepted);
		});
	}
	...
	private localStorageHelper = (() => {
		...
		const checkOneSignalIds = (getOneSignalId: string | null, getSubscriptionId: string | null) => {
			const storedObject = this._local.getItem('ONE_SIGNAL_NOTIFICATION_DETAILS');

			if (storedObject) {
				const oneSignalObj = JSON.parse(storedObject);
				const isOneSignalIdMatch = oneSignalObj.ONE_SIGNAL_ID === getOneSignalId;
				const isSubscriptionIdMatch = oneSignalObj.SUBSCRIPTION_ID === getSubscriptionId;
		...
		// Public API
		return {
			verifyIds: (getOneSignalId: string | null, getSubscriptionId: string | null) => {
	...
	storeNotificationDetails(id: string) {
		const idsMatch = this.localStorageHelper.verifyIds(this.onsSignalID, this.onsSignalSubscriptionID);
		if (idsMatch) {
